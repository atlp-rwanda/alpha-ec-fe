...
import { FC_UserRegistration } from "@/services/authenticate";
...
export default function Home() {
  ...
  const getErrorForField = (key: RegistrationKeys) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await FC_UserRegistration(data);
    ...
    <div className="w-2/3 mx-auto flex flex-col align-middle animate__animated animate__backInLeft ">
      ...
      >
        {RegistrationFields.map((field, i) => (
          <div className="w-full mt-2 flex flex-col gap-1 animate__animated animate__fadeInDown" key={i}>
            <InputElement
              label={field.label}
